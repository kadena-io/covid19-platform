...
(module covid GOVERNANCE
  ...
  (defun register-test (printer-pub-key:string test-pub-key:string ks-name:string test-manufacturer:string test-model:string)
    ...
    (with-capability (PRINTING-ENTITY printer-pub-key)
      (coin.create-account test-pub-key (read-keyset ks-name))
      (insert test-table-three test-pub-key {
      ...
      (with-read printer-table-three printer-pub-key {
        ...
        (update printer-table-three printer-pub-key {
          "test-pub-keys": (+ test-pub-keys [test-pub-key])
        })
      )
      (format
        "Test with pub-key={} registered on chain by printer={}"
        [test-pub-key, printer-pub-key])
    )
  )
